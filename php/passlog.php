...
public function passLog() {
    ...
    echo '<div id="dialog_object"><div id="dialogs_block" onclick="dialogsBlockClick(this)"><div id="dialogs_id">'.$all_dialogs[$i].'</div>
    <div id="dialogs_login">'.$full_dia_with.'</div><br><br>
    <div id="dialogs_message">Нет сообщений</div></div><div id="date_block">'.$row2['date'].'</div>
    <button id="deletebutton" onclick="deleteButton(this)">Удалить</button></div><br><br>';

    } else {
    echo '<div id="dialog_object"><div id="dialogs_block" onclick="dialogsBlockClick(this)"><div id="dialogs_id">'.$all_dialogs[$i].'</div>
    <div id="dialogs_login">'.$full_dia_with.'</div><br><br><div id="dialogs_message">
    '.$row2['message'].'</div></div><div id="date_block">'.$row2['date'].'</div>
    <button id="deletebutton" onclick="deleteButton(this)">Удалить</button></div><br><br>';
        }
